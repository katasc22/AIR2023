    ...
    def runExperiment(self):
        ...
        if self.experiment_approach == "all":
            ...
            if self.experiment_mode == "monolingual":
                retrieved_docs_per_query = retrieve_k_documents_per_query_tb_monolingual(self.preprocessed_data.queries,
                                                                                         self.preprocessed_data.docs,
            ...
            retrieved_docs_per_query = retrieve_k_documents_per_query_mbert(self.preprocessed_data.queries,
                                                                            self.preprocessed_data.docs,
                                                                            15, device=self.device)
            return (ExperimentResultData("ml-mbert", retrieved_docs_per_query, 15),)

        elif self.experiment_approach == "ml_knowledge_distillation":
            retrieved_docs_per_query = retrieve_k_documents_per_query_distiluse(self.preprocessed_data.queries,
                                                                                self.preprocessed_data.docs,
                                                                                15, device=self.device)
